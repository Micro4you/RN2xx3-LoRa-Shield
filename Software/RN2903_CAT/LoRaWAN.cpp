...
LoRaWAN::LoRaWAN(byte RX, byte TX):loraSerial(RX,TX)         // RX, TX   ** Set to 8, 9 for Arduino Uno R3 board
{
...
void LoRaWAN::LoRaNetworkSessionKey(char* nKey)
{
  ...
  //waitCommandResponse();
  //loraSerial.println(F("mac get nwkskey"));
  //waitCommandResponse();
  //Serial.print(F("Network Session Key Set To: "));Serial.println(_rxBuffer);
...
void LoRaWAN::LoRaChannelPlan(byte st_ch, byte end_ch)
{
  ...
  for (int x=0; x<64; x++)    //turn all channels off
  {
...
void LoRaWAN::LoRaShowConfig()
{
  ...
  for (int x=0; x<64; x++)
  {
    ...
    if (_rxBuffer[0]=='o' && _rxBuffer[1]=='n')     //check if channel is on, acquire rest of info
    {
        ...
        waitCommandResponse();
        dRange = 0;
        if (_rxBuffer[0]=='0' && _rxBuffer[2]=='5')
          {
            dRange = 125;
          }
        ...
        Serial.print(F("Channel "));Serial.print(x);Serial.print(F(": Frequency = "));Serial.print(ch_freq);Serial.print(F("Mhz, DataRate = "));Serial.print(dRange);Serial.println(F("kHz"));
    }
   }
